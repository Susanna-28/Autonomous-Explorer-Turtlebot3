...
class Explore:
    ...
    def save_positions_to_excel(self):
        ...
        with self.save_lock:
            ...
            valid_positions = [pos for pos in self.positions if len(pos) == 2]
            if valid_positions:  # Salva solo se ci sono dati validi
                df = pd.DataFrame(valid_positions, columns=['x', 'y'])
                df.to_excel('/root/ws/progetto/catkin_ws/src/pkg_project/positions_frontier.xlsx', index=False)
                rospy.loginfo("Posizioni salvate in positions_frontier.xlsx")
    ...
    def map_callback(self,msg):
        ...
        self.explored_cells = sum(1 for cell in grid_data if cell == 0)  # Conta le celle libere (valore 0)
    ...
    def stop_robot_if_needed(self):
        if self.total_cells > 0:
            ...
            rospy.loginfo(f"Percentuale esplorata: {self.percentage_explored}%")  # Log per debug
            if self.percentage_explored >= 85 and self.navigation_active:
                self.stop_robot()
                rospy.loginfo("Robot fermato perché la percentuale di area esplorata ha superato l'85%.")
                self.navigation_active = False  # Disattiva la navigazione una volta fermato
            elif not self.navigation_active:
                self.stop_robot()  # Assicura che il robot continui a rimanere fermo
    ...
    def print_exploration_stats(self):
        ...
        if self.total_cells > 0:
            ...
            rospy.loginfo(f"Percentuale area esplorata: {percentage_explored:.2f}% ({self.explored_cells}/{self.total_cells})")
